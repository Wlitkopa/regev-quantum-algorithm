...
def draw_plot(N_list, d_qd_list, type_of_test_list, meas_R_list):
    ...
    for d_qd in d_qd_list:
        ...
        # for type_of_test in type_of_test_list:
            ...
            # ALL OUTPUT REGS CONFIGURATIONS AND ALL R PARAMETER VALUES
            ...
            # ALL TEST TYPES FOR A GIVEN OUTPUT REGISTER CONFIGURATION AND R CONFIGURATION
            plt.plot(N_list[:len(effectiveness_all_list)], effectiveness_all_list,
                     label=f"Square root of unity modulo N, test type {type_of_test}",
                     color=colors[color_id - 1][0], marker='o')
            plt.plot(N_list[:len(effectiveness_nontrivial_list)], effectiveness_nontrivial_list,
                     label=f"Non-trivial square root of unity modulo N, test type {type_of_test}",
                     color=colors[color_id - 1][1], marker='o')
        ...
        # ALL TEST TYPES FOR A GIVEN OUTPUT REGISTER CONFIGURATION AND R CONFIGURATION
        plt.title(f"Effectiveness for {d}_{qd}, {output_register_measuring} output registry measuring, {R_type} R")
        ...
        # ALL TEST TYPES FOR A GIVEN OUTPUT REGISTER CONFIGURATION AND R CONFIGURATION
        # plt.savefig(f"../../../images/plots/effectiveness_gauss/effectiveness_{d}_{qd}_all_test_types_{output_register_measuring}_{R_type}.svg", format="svg")
        plt.savefig(f"../../../images/plots/effectiveness_gauss/effectiveness_{d}_{qd}_all_test_types_{output_register_measuring}_{R_type}.png", format="png")
        print(f"CREATED PLOT: images/plots/effectiveness_gauss/effectiveness_{d}_{qd}_all_test_types_{output_register_measuring}_{R_type}.png")
